...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    const compactRouteForPayloadRules =
      req.path === '/responses/compact' ||
      req.path === '/v1/responses/compact' ||
      (req.originalUrl && req.originalUrl.includes('/responses/compact'))
    const shouldUseOpenAIResponsesToggles =
      !isChatCompletionsCompat && !compactRouteForPayloadRules && !req._fromUnifiedEndpoint
    ...
    if (
      ...
      !isCodexModel &&
      (!shouldUseOpenAIResponsesToggles ||
        apiKeyData.enableOpenAIResponsesCodexAdaptation !== false)
    ) {
    ...
    let clientWantsStream = isChatCompletionsCompat
      ? originalRequestBody?.stream === true
      : req.body?.stream !== false
    let upstreamUseStream = isChatCompletionsCompat ? true : clientWantsStream
    ...
    const shouldApplyCodexAdaptation = shouldUseOpenAIResponsesToggles
      ? apiKeyData.enableOpenAIResponsesCodexAdaptation !== false && !shouldPassThroughCodex
      : !shouldPassThroughCodex && !isChatCompletionsCompat
    ...
    if (shouldUseOpenAIResponsesToggles && apiKeyData.enableOpenAIResponsesPayloadRules === true) {
      req.body = requestBodyRuleService.applyRules(req.body, apiKeyData.openaiResponsesPayloadRules)
      requestedModel = req.body?.model || requestedModel
      if (!isChatCompletionsCompat) {
        clientWantsStream = req.body?.stream !== false
        upstreamUseStream = clientWantsStream
      }
