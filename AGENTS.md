...
这个文件为 Codex (Codex.ai/code) 提供在此代码库中工作的指导。

## 项目概述

Codex Relay Service 是一个多平台 AI API 中转服务，支持 **Codex (官方/Console)、Gemini、OpenAI Responses (Codex)、AWS Bedrock、Azure OpenAI、Droid (Factory.ai)、CCR** 等多种账户类型。提供完整的多账户管理、API Key 认证、代理配置、用户管理、LDAP认证、Webhook通知和现代化 Web 管理界面。该服务作为客户端（如 Codex、Gemini CLI、Codex、Droid CLI、Cherry Studio 等）与 AI API 之间的中间件，提供认证、限流、监控、定价计算、成本统计等功能。
...
- **多账户类型支持**: 支持 Codex-official、Codex-console、bedrock、ccr、droid、gemini、openai-responses、azure-openai 等账户类型
...
- **权限控制**: API Key支持权限配置（all/Codex/gemini/openai等），控制可访问的服务类型
- **客户端限制**: 基于User-Agent的客户端识别和限制，支持Codex、Gemini-CLI等预定义客户端
- **模型黑名单**: 支持API Key级别的模型访问限制
- **并发请求排队**: 当API Key并发数超限时，请求进入队列等待而非立即返回429，支持配置最大排队数、超时时间，适用于Codex Agent并行工具调用场景

### 主要服务组件

#### 核心转发服务

- **claudeRelayService.js**: Codex官方API转发，处理OAuth认证和流式响应
- **claudeConsoleRelayService.js**: Codex Console账户转发服务
...
- **claudeAccountService.js**: Codex官方账户管理，OAuth token刷新和账户选择
- **claudeConsoleAccountService.js**: Codex Console账户管理
...
- **unifiedClaudeScheduler.js**: Codex多账户类型统一调度（Codex-official/console/bedrock/ccr）
...
- **claudeCodeHeadersService.js**: Codex客户端请求头处理

#### 工具服务

- **oauthHelper.js**: OAuth工具，PKCE流程实现和代理支持
- **workosOAuthHelper.js**: WorkOS OAuth集成
- **openaiToClaude.js**: OpenAI格式到Codex格式的转换

### 认证和代理流程

1. 客户端使用自建API Key（cr\_前缀格式）发送请求到对应路由（/api、/Codex、/gemini、/openai、/droid等）
...
- ✅ **Codex Console账户**: 支持Codex Console类型账户
...
- ✅ **权限控制**: API Key级别的服务权限（all/Codex/gemini/openai）
...
- `CLAUDE_OVERLOAD_HANDLING_MINUTES`: Codex 529错误处理持续时间（分钟，0表示禁用）
...
- **多平台账户管理**:
  - Codex账户（官方/Console）: OAuth账户添加、代理配置、状态监控
...
- `POST /openai/Codex/v1/chat/completions` - OpenAI格式转Codex
- `POST /openai/gemini/v1/chat/completions` - OpenAI格式转Gemini
- `GET /openai/v1/models` - OpenAI格式模型列表

#### Droid (Factory.ai) 路由
- `POST /droid/Codex/v1/messages` - Droid Codex转发
...
#### OAuth和账户管理
- `POST /admin/Codex-accounts/generate-auth-url` - 生成OAuth授权URL（含代理）
- `POST /admin/Codex-accounts/exchange-code` - 交换authorization code
- `POST /admin/Codex-accounts` - 创建Codex OAuth账户
...
- 检查日志文件 `logs/Codex-relay-*.log` 确认服务正常运行
...
- **多账户类型支持**: 支持8种账户类型（Codex-official、Codex-console、bedrock、ccr、droid、gemini、openai-responses、azure-openai）
...
- **权限系统**: API Key支持细粒度权限控制（all/Codex/gemini/openai等）
...
- **并发请求排队**: 当API Key并发超限时，请求进入队列等待而非直接返回429
  ...
  - **适用场景**: Codex Agent并行工具调用、批量请求处理
- **客户端识别**: 基于User-Agent的客户端限制，支持预定义客户端（Codex、Gemini-CLI等）
...
- **账户数据**（多类型）:
  - `claude_account:{id}` - Codex官方账户（加密的OAuth数据）
  - `claude_console_account:{id}` - Codex Console账户
